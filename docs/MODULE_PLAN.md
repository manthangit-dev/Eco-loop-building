...
Modules 0 and 1 are completed. Every later module is pending and must not be implemented early.
...
| 2 | Baseline EnergyPlus building — Pending | Establish baseline office model | Original IDF, weather | Preserved source and runnable derived setup | 1 | Real baseline run | Model runs with declared inputs |
| 3 | Python EnergyPlus runner — Pending | Launch and coordinate simulation | Environment, model | Minimal runner interface | 1, 2 | Executed runner check | Runner starts simulation reproducibly |
